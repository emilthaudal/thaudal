...
async function fetchContributions(
  ...
  if (!res.ok) {
    ...
    throw new Error(`GitHub GraphQL error for ${login}: ${res.status} ${body}`)
  }
...
// Fetches both accounts' contribution calendars in parallel and merges them
// by date. Individual `fetch` calls are cached by Next.js for an hour, so
// calling this from multiple places in the same render is cheap.
export async function getGithubActivity(): Promise<ActivityResponse | null> {
